...
1. Kjerneverdier prefixes med `--rds` skal alltid finnes i begge brands
1. LightMode skal kunne settes direkte på komponenten, eller ved å være et child av klassen `rds-light`
...
Ny release lages ved å oppdatere versjonsnummeret i `shared-components/package.json` og merge den til master med commit meldingen `chore: {descripe changes} [release]`, release til npm skjer automatisk.
